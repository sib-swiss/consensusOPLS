...
discriminant analysis on the `demo_data` set proposed in [@BOCCARD2013]. This 
analysis concerned a selection of data from a publicly available repository of 
the National Cancer Institute, i.e. the NCI-60 data set. This data set provides 
experimental data obtained from 60 human cancer cell lines derived from 9 tissue 
origins [@SHOEMAKER2006]. Two tissue were selected in this study (colon versus 
ovary), and three data sources were chosen: transcriptomics,proteomics and 
metabolomics data (3 blocks of data, evaluated on the same observations):

- Metabolomics: 14x 150 variables
- Microarray: 14x 200 variables
- Proteomics: 14x 100 variables

The combination of these data sources was expected to provide a global profiling 
...
As mentioned earlier, we use the demonstration dataset proposed by [@BOCCARD2013]:
...
- the R-squared (`R2Y`) coefficient, gives a measure of how predictive the 
...
paste0('R2Y: ', round(Optimal_model_no_perm$modelCV$Model$R2Yhat[position], 4))
```
...
latent variables calculated by the optimal model. A horizontal separation is 
expected.
...
```{r plot_bloc_PredVSOrtho, message = FALSE, class.source='fold-hide'}
...
The loading plot shows the projection of variables in the the space of the two 
latent variables calculated by the optimal model.
...
Finally, for the optimal model selected before permutations, it is possible to 
observe the VIPs and loadings for the first predictive component.
...
Note that VIP = 1 corresponds to the average VIP, meaning that VIPs > 1 are 
considered the most important for the model. In fact, we can use 1 as a 
threshold on VIPs to restrict the study to the most predictive variables. A blue 
vertical line has been added to view this threshold.
...
On all the graphs below, a blue vertical line has been added to represent the 
optimal model. Since it lies far from the distribution of permuted models, it 
implies that the signal is indeed linked to the data and not to chance.
