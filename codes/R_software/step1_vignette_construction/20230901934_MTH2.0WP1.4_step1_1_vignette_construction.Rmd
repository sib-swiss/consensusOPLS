...
well as many graphics (scores plot, block contributions, loadings, permutations, 
etc.).
...
The profiles seem to differ according to the Y response variable.
...
`r round(RVConsensusOPLSModelCV$Model$R2Yhat[position], 4)*100`$\%$ of the 
...
the variables in relation to both response and projection quality. The 20 most 
relevant features were selected, using `VIP* sign(loadings)` value.
...
VIP_plot <- lapply(X = 1:nbrBlocs,
                         FUN = function(X){
                           signe_loadings <- sign(RVConsensusOPLSModelCV$Model$loadings[[X]][, "p_1"])
                           VIP[[X]]*signe_loadings})
names(VIP_plot) <- demo_data$BlockNames

# Metabo data
ggplot2::ggplot(data = data.frame("variables" = names(sort(VIP_plot[[1]], decreasing = TRUE))[1:20], 
                                  "valeur" = sort(VIP_plot[[1]], decreasing = TRUE)[1:20]), 
                aes(x = variables, y = valeur)) +
  ggplot2::geom_bar(stat = "identity") +
  ggplot2::labs(title = paste0("Barplot of ",demo_data$BlockNames[1])) +
  ggplot2::xlab("Top 20 of predictive variables") +
  ggplot2::ylab("VIP x loading sign") +
  theme_graphs +
  ggplot2::theme(axis.text.x = element_text(angle = 45, vjust = 1, hjust = 1)) 

# Microarray data
ggplot2::ggplot(data = data.frame("variables" = names(sort(VIP_plot[[2]], decreasing = TRUE))[1:20], 
                                  "valeur" = sort(VIP_plot[[2]], decreasing = TRUE)[1:20]), 
                aes(x = variables, y = valeur)) +
  ggplot2::geom_bar(stat = "identity") +
  ggplot2::labs(title = paste0("Barplot of ",demo_data$BlockNames[2])) +
  ggplot2::xlab("Top 20 of predictive variables") +
  ggplot2::ylab("VIP x loading sign") +
  theme_graphs +
  ggplot2::theme(axis.text.x = element_text(angle = 45, vjust = 1, hjust = 1)) 
...
# Proteo data
ggplot2::ggplot(data = data.frame("variables" = names(sort(VIP_plot[[3]], decreasing = TRUE))[1:20], 
                                  "valeur" = sort(VIP_plot[[3]], decreasing = TRUE)[1:20]), 
                aes(x = variables, y = valeur)) +
  ggplot2::geom_bar(stat = "identity") +
  ggplot2::labs(title = paste0("Barplot of ",demo_data$BlockNames[3])) +
  ggplot2::xlab("Top 20 of predictive variables") +
  ggplot2::ylab("VIP x loading sign") +
  theme_graphs +
  ggplot2::theme(axis.text.x = element_text(angle = 45, vjust = 1, hjust = 1)) 
...
plot_bloc_pred <- ggplot2::ggplot(data = 
                                    data.frame("Values" = RVConsensusOPLSModelCV$Model$lambda[,1],
...
plot_bloc_1stOcomp <- ggplot2::ggplot(data = 
                                        data.frame("Values" = RVConsensusOPLSModelCV$Model$lambda[,2],
                                                   "Blocks" = as.factor(labels(demo_3_Omics[1:nbrBlocs]))),
                                      aes(x = Blocks, y = Values,
                                          fill = Blocks, labels = Values)) +
  ggplot2::geom_bar(stat = 'identity') + 
...
data_two_plots <- data.frame("Values" = c(data_two_plots$Values,
                             ...
                             "Type" = c(data_two_plots$Type,
                                        rep("Ortho", times = length(RVConsensusOPLSModelCV$Model$lambda[,2]))),
...
data_PredVSOrtho <- ggplot2::ggplot(data = data.frame("Pred" = RVConsensusOPLSModelCV$Model$lambda[,1],
...
Individual loadings of each block were calculated for the predictive latent 
...
```{r plot1_perm, message = FALSE, class.source='fold-hide'}
PermRes$plots$R2val
```

```{r plot2_perm, message = FALSE, class.source='fold-hide'}
PermRes$plots$Q2val
```

```{r plot3_perm, message = FALSE, class.source='fold-hide'}
PermRes$plots$R2_and_Q2
```

```{r plot4_perm, message = FALSE, class.source='fold-hide'}
PermRes$plots$R2val_hist
```

```{r plot5_perm, message = FALSE, class.source='fold-hide'}
PermRes$plots$Q2val_hist
```

```{r plot6_perm, message = FALSE, class.source='fold-hide'}
