...
demo_3_Omics <- 
  lapply(X = 1:length(demo_data$BlockNames), 
         FUN = function(X){
           # Extract the tables of data
           name_data <- demo_data$BlockNames[X]
           name_var <- gsub("Data", "VarNames", name_data, fixed = TRUE)
           
           # Extract the corresponding variables names
           colnames(demo_data[[name_data]]) <- as.character(unlist(demo_data[[name_var]][1, ]))
           data <- demo_data[[name_data]]
           
           # Extract columns length for all subjects
           col_identic_val <- base::apply(X = data,
                                          MARGIN = 2, 
                                          FUN = function(X){
...
We then export these results for comparison with the matlab version. The user c
an then load the data directly into R, without using the `R.matlab` package.
...
To repeat the example in the demo code, the datasets are centered and reduced. 
Then the `matrix2saisir` function is applied to each of them:

```{r data_form}
demo_3_Omics[1:length(demo_data$BlockNames)] <- 
  lapply(X = 1:length(demo_data$BlockNames), 
         FUN = function(X){
           # Calculate the Z-score 
           # (mean centering and standard deviations scaling)
           scaled_data <- base::scale(demo_3_Omics[X][[1]], 
                                      center = TRUE, scale = TRUE)
           
           # Apply the function matrix2saisir
           matrix <- matrix2saisir(data = scaled_data,
                                   coderow = NA, codecol = NA,
                                   position_coderow = NA, position_codecol = NA)
...
the source code demonstrating the use of the method. The dimensions of the 
data after verification were therefore:

```{r check_dim, class.source='fold-hide'}
lapply(X = 1:length(demo_3_Omics),
...
RVConsensusOPLSModelCV <- RVConsensusOPLS(data = demo_3_Omics[1:nbrBlocs],
                                          Y = demo_3_Omics$Y,
                                          A = LVsPred,
...
VIP <- MBVIP(collection = demo_3_Omics[1:nbrBlocs], Y = demo_3_Omics$Y, 
...
PermRes <- RVConsensusOPLSPerm(data = demo_3_Omics[1:nbrBlocs],
                               Y = demo_3_Omics$Y,
                               nbruns = 1000,
                               PredLVs = LVsPred,
                               maxOrtholvs = LVsOrtho)
