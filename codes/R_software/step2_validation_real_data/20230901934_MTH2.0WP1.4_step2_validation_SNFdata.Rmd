...
#   file = paste0(path_data_test, common_name, common_RV, "01_RV.txt"),
#   header = TRUE, sep = "\t", dec = "."))[1,],
#   rvcopls$optimal$modelCV$RV, tolerance=1e-3)
...
matlab <- as.matrix(read.table(
  ...
  header = TRUE, sep = "\t", dec = ".", row.names = 1))
R_res <- rvcopls$optimal$modelCV$Model$blockContribution
relative_compare <- (matlab - R_res)/ max(1, R_res)
matrix_compare <- matrix(0, ncol = ncol(relative_compare),
                         nrow = nrow(relative_compare))
colnames(matrix_compare) = colnames(relative_compare)
...
#   file = paste0(path_data_test, common_name, common_RV, "05_lambda.txt"),
#   header = TRUE, sep = "\t", dec = ".", row.names = 1)),
#   rvcopls$optimal$modelCV$Model$blockContribution, tolerance=1e-3)



## loadings
for(i in 1:length(nbrBlocs)){
  loading_R <- rvcopls$optimal$modelCV$Model$loadings[[1]]
...
rownames(Cp_matlab) <- c("Breast", "Lung")
...
colnames(AllYhat_matlab) <- gsub(pattern = "^1", replacement = "Breast",
                                 x = colnames(AllYhat_matlab))
colnames(AllYhat_matlab) <- gsub(pattern = "^2", replacement = "Lung",
                                 x = colnames(AllYhat_matlab))
AllYhat_R <- rvcopls$optimal$modelCV$cv$AllYhat
relative_compare <- (AllYhat_matlab - AllYhat_R)/ max(1, AllYhat_R)
matrix_compare <- matrix(0, ncol = ncol(relative_compare),
                         nrow = nrow(relative_compare))
colnames(matrix_compare) = colnames(relative_compare)
rownames(matrix_compare) = rownames(relative_compare)
...
#                        rvcopls$optimal$modelCV$cv$AllYhat, tolerance=1e-6)
...
matlab <- as.numeric(as.matrix(read.table(
  ...
  header = FALSE, sep = ",", dec = ".")))
R_res <- rvcopls$optimal$modelCV$cv$Q2Yhat
relative_compare <- (AllYhat_matlab - AllYhat_R)/ max(1, AllYhat_R)
matrix_compare <- matrix(0, ncol = ncol(relative_compare),
                         nrow = nrow(relative_compare))
colnames(matrix_compare) = colnames(relative_compare)
...
#   file = paste0(path_data_test, common_name, common_CV, "05_Q2Yhat.txt"),
#   header = FALSE, sep = ",", dec = "."))),
#   rvcopls$optimal$modelCV$cv$Q2Yhat, tolerance=1e-6)
...
matlab <- as.numeric(as.matrix(read.table(
  ...
  header = FALSE, sep = ",", dec = ".")))
R_res <- rvcopls$optimal$modelCV$cv$DQ2Yhat
relative_compare <- (AllYhat_matlab - AllYhat_R)/ max(1, AllYhat_R)
matrix_compare <- matrix(0, ncol = ncol(relative_compare),
                         nrow = nrow(relative_compare))
colnames(matrix_compare) = colnames(relative_compare)
...
#   file = paste0(path_data_test, common_name, common_CV, "06_DQ2Yhat.txt"),
#   header = FALSE, sep = ",", dec = "."))),
#   rvcopls$optimal$modelCV$cv$DQ2Yhat, tolerance=1e-6)
...
matlab <- read.table(file =
                     ...
                     header = TRUE, sep = "\t", dec = ".",
                     row.names = 1)
R_res <- rvcopls$optimal$modelCV$Model$scores
...
#                                   header = TRUE, sep = "\t", dec = ".",
#                                   row.names = 1),
#                        rvcopls$optimal$modelCV$Model$scores,
#                        tolerance=1e-6)
``` 
...
ggplot2::ggplot(data = data.frame(
  "p_1" = Optimal_model_no_perm$modelCV$Model$scoresP,
