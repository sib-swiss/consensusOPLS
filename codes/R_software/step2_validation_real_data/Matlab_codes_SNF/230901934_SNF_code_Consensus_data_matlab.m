...
data = load('20230901934_step2_validation_data_SNF_common_collection.mat');
data = data.data_SNF;
%Ouverture du fichier de noms des colonnes des données
VarNames = load('20230901934_step2_validation_data_SNF_common_VarNames.mat');
VarNames = VarNames.data_SNF_VarNames;
%Ouverture du fichier du facteur Y
Y = load('20230901934_step2_validation_data_SNF_common_Y_factor.mat');
Y = Y.Y;
% Conversion Y en numerique
valeurs_numeriques = zeros(size(Y));
for i = 1:numel(Y)
    % Vérifie si la chaîne de caractères est "Breast"
    if strcmpi(Y{i}, 'Breast')
        % Associer la valeur numérique 1
        valeurs_numeriques(i) = 1;
    % Vérifie si la chaîne de caractères est "Lung"
    elseif strcmpi(Y{i}, 'Lung')
        % Associer la valeur numérique 2
        valeurs_numeriques(i) = 2;
    end
end
Y = valeurs_numeriques;
%Ouverture du fichier des noms d'echantillons
Sample_names = load('20230901934_step2_validation_data_SNF_common_SampleNames.mat');
...
collection_SNF(1) = matrix2saisir(data.Gene);
collection_SNF(2) = matrix2saisir(data.Methy);
collection_SNF(3) = matrix2saisir(data.Mirna);
%Modification des noms de variables de la collection
collection_SNF(1).i = VarNames.Gene;
collection_SNF(2).i = VarNames.Methy;
collection_SNF(3).i = VarNames.Mirna;
% Sauvegarder la structure en format .mat
save('20230901934_step2_validation_data_SNF_common_collection_matlab.mat', 'collection_SNF')
...
CVfolds=size(collection_SNF(1).d,1); % Number of cross-validation folds

collection = collection_SNF;
Y = Y; 
A = LVsPred; 
...
text(RVConsensusOPLSModelCV.koplsModel.T(:,1), ...
    ...
    'bottom','FontSize', 8)
saveas(gcf,'20230901934_step2_validation_data_SNF_common_scoresPlot.png')
...
saveas(gcf,'20230901934_step2_validation_data_SNF_common_BlocContribToPredCompo.png')
...
saveas(gcf,'20230901934_step2_validation_data_SNF_common_BlocContribTo1stOrthoCompo.png')
...
saveas(gcf,'20230901934_step2_validation_data_SNF_common_BlocContribPredVSOrthoCompo.png')
...
legend(gca,'show')
saveas(gcf,'20230901934_step2_validation_data_SNF_common_LoadingPlot.png')


%% Permutations
tStart = clock; 
[PermRes]=RVConsensusOPLSPerm(collection,Y,1000,LVsPred,LVsOrtho,Sample_names, Blocs_Names);
tStop = clock;
...
writetable(scores, '20230901934_step2_validation_data_SNF_common_scores.txt');
...
writetable(loadings_VIP_bloc1, '20230901934_step2_validation_data_SNF_common_loadings_VIP_Ceramids.txt');
...
writetable(loadings_VIP_bloc2, '20230901934_step2_validation_data_SNF_common_loadings_VIP_Eicosanoids.txt');
...
writetable(loadings_VIP_bloc3, '20230901934_step2_validation_data_SNF_common_loadings_VIP_NegLcMs.txt');
...
writetable(loadings_VIP_bloc4, '20230901934_step2_validation_data_SNF_common_loadings_VIP_NegSfcMs.txt');
...
writetable(loadings_VIP_bloc5, '20230901934_step2_validation_data_SNF_common_loadings_VIP_Nmr.txt');
...
writetable(loadings_VIP_bloc6, '20230901934_step2_validation_data_SNF_common_loadings_VIP_PosLcMs.txt');
...
writetable(loadings_VIP_bloc7, '20230901934_step2_validation_data_SNF_common_loadings_VIP_PosSfcMs.txt');