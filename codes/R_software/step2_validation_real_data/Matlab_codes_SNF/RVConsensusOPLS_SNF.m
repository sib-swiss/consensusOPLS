...
for ta=1:ntable
	...
	if(ta==1)
		...
		writetable(temp_table, "20230901934_SNF_RVConsensusOPLS_00_temp.txt", 'Delimiter', '\t', 'WriteRowNames', true);	
	end
end
AMat_table = cell(1, 7);
for i = 1:numel(AMat)
    ...
    filename = ['20230901934_SNF_RVConsensusOPLS_02_Amat_', num2str(i), '.txt'];
    writetable(AMat_table_tmp, filename, 'Delimiter', '\t', 'WriteRowNames', true);
end
RV_table = array2table(RV, 'VariableNames', Blocs_Names);
writetable(RV_table, "20230901934_SNF_RVConsensusOPLS_01_RV.txt", 'Delimiter', '\t');
%W_mat_table = array2table(W_mat, 'RowNames', Sample_names, 'VariableNames', Sample_names);
%writetable(W_mat_table, "20230901934_SNF_RVConsensusOPLS_03_Wmat.txt", 'Delimiter', '\t', 'WriteRowNames', true);
...
writematrix(modelCV.koplsModel.Cp, "20230901934_SNF_ConsensusOPLSCV_01_Cp.txt")
writematrix(modelCV.koplsModel.Sp, "20230901934_SNF_ConsensusOPLSCV_02_Sp.txt")
writematrix(modelCV.koplsModel.Up, "20230901934_SNF_ConsensusOPLSCV_03_Up.txt")

AllYhat_table = array2table(modelCV.cv.AllYhat, 'RowNames', Sample_names, 'VariableNames', ["1_p" "2_p" "1_po1" "2_po1"]);
writetable(AllYhat_table, '20230901934_SNF_ConsensusOPLSCV_04_AllYhat.txt', 'Delimiter', '\t', 'WriteRowNames', true);

writematrix(modelCV.cv.Q2Yhat, "20230901934_SNF_ConsensusOPLSCV_05_Q2Yhat.txt")
writematrix(modelCV.cv.DQ2Yhat, "20230901934_SNF_ConsensusOPLSCV_06_DQ2Yhat.txt")
...
for i = 1:numel(modelCV.koplsModel.Tp)
    ...
    filename = ['20230901934_SNF_ConsensusOPLSCV_07_Tp_', num2str(i), '.txt'];
...
writetable(scores_table, '20230901934_SNF_ConsensusOPLSCV_08_scores.txt', 'Delimiter', '\t', 'WriteRowNames', true);
...
writetable(lambda_raw_table, '20230901934_SNF_RVConsensusOPLS_04_lambdaRaw.txt', 'Delimiter', '\t', 'WriteRowNames', true);
...
writetable(lambda_table, '20230901934_SNF_RVConsensusOPLS_05_lambda.txt', 'Delimiter', '\t', 'WriteRowNames', true);
...
for i = 1:numel(Blocs_Names)
    ...
    filename = ['20230901934_SNF_RVConsensusOPLS_06_loadings_', num2str(i), '.txt'];
